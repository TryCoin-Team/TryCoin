...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "TryCoin " + FormatFullVersion();
...
void static StartSync(const vector<CNode*> &vNodes) {
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        ...
        if (!pnode->fClient && !pnode->fOneShot &&
            ...
            (pnode->nVersion < NOTCOS_VERSION_START || pnode->nVersion >= NOTCOS_VERSION_END)) {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TryCoin is probably already running."), addrBind.ToString());
        else
