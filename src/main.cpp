...
#if defined(NDEBUG)
# error "TryCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "TryCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if ( pindexPrev->nTime > 1593532800)
        nSubsidy = COIN * 50;
    else
        nSubsidy = COIN + nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile tcodat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && tcodat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(tcodat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), tcodat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
